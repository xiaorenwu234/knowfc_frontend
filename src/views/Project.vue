                ...
                <div class="w-full md:w-80 flex-shrink-0 pt-8">
                    <div class="sticky top-32">
                        ...
                        <div class="space-y-3">
                            ...
                            <button
                                v-if="user && user.id && project.owner && project.owner.id && user.id !== project.owner.id"
                            ...
                            <button
                                class="w-full border border-gray-300 text-gray-700 py-2 px-4 rounded-xl hover:bg-gray-50 transition-colors shadow"
                                @click="openInviteDialog">邀请成员</button>
                        </div>
                        <!-- 邀请弹窗 -->
                        <div v-if="showInviteDialog"
                            ...
                            <div class="bg-white rounded-xl p-6 w-[500px] max-h-[80vh] overflow-y-auto shadow-xl">
                                ...
                                <!-- 搜索结果 -->
                                <div v-if="searchResults.length === 0" class="text-gray-400 text-sm mb-2">暂无搜索结果</div>
                                <ul>
                                ...
                                <div class="flex justify-end gap-2 mt-4">
                                    ...
                                    <button @click="sendInvites"
                                        class="px-4 py-1 rounded bg-green-600 text-white hover:bg-green-700">发送邀请</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
const isJoined = computed(() => {
    ...
    if (project.value.owner?.id === user.value.id) return true
    return project.value.members?.some(m => m.id === user.value.id)
})
...
async function applyProject() {
    ...
    try {
        ...
        formData.append('applicantId', user.value.id)
...
async function sendInvites() {
  ...
  for (const inviteeId of selectedUserIds.value) {
    await axios.post(buildApiUrl('/project/invite'), {
      inviterId,
      inviteeId,
      projectId
    }, {
