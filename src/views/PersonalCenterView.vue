...
const submitCreate = async () => {
  if (!createForm.value.name.trim() || !createForm.value.projectInfo.trim() || !createForm.value.cooperationTerms.trim()) {
...
<template>
  <div class="w-screen h-screen pt-24 bg-white z-10">
    <div class="flex flex-col sm:flex-row w-full h-full p-6 sm:p-0">
      <div class="flex sm:flex-col items-center sm:items-start sm:w-[28%] sm:min-h-[250px] gap-4 sm:gap-0">
        <div class="w-full sm:flex sm:justify-center">
          <img class="w-[140px] aspect-square sm:w-[calc(100%-50px)] rounded-full border-2 mx-auto sm:mx-0 mt-4"
            src="/image.png" alt="头像" />
        </div>
      ...
      <div class="flex-1 mt-4">
        <div>
          <div class="font-bold text-xl">{{ ownerReference + '的科研项目' }}
  ...
  <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white p-6 rounded-lg w-full max-w-md mx-4">
      ...
      <input v-model="reason" type="text" placeholder="输入申请理由"
...
<div v-if="showCreateModal" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
  <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-8 relative animate-fade-in">
    ...
    <div class="space-y-4">
      <input v-model="createForm.name" type="text" placeholder="项目名称"
      ...
      <input v-model="createForm.projectInfo" type="text" placeholder="项目描述"
        class="w-full px-4 py-3 border border-gray-200 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition outline-none text-base" />
      <input v-model="createForm.cooperationTerms" type="text" placeholder="合作条件"
        class="w-full px-4 py-3 border border-gray-200 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition outline-none text-base" />
